...
def fetch(stop_crops, min_stop_crops):
    ...
    for si in SHARD_ORDER[:MAX_SHARDS]:
        ...
        try:
            ...
            cols = [c for c in ("image", "crop", "disease") if c in available]
        except Exception:
            cols = ["image", "crop", "disease"]  # force safe columns, never load all
        for batch in tqdm(pf.iter_batches(batch_size=64, columns=cols),
                          total=pf.metadata.num_rows // 64 + 1, desc=f"shard{si:04d}"):
            d = batch.to_pydict()
...
def exp3_finetune_wiseft(tier, seen, unseen, device, ft_epochs=5, alphas=(0.0, 0.5, 1.0)):
    ...
    def embed_rows(rows):
        ...
        dl = DataLoader(E(), batch_size=BATCH, num_workers=0)
        embs, labs = [], []
        model.eval()
        with torch.no_grad():
            for px, lab in dl:
    ...
    dl = DataLoader(TrainDS(), batch_size=BATCH, shuffle=True, num_workers=0)
